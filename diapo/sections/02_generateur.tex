...
\begin{frame}
    ...
    \begin{block}{Modèle du générateur}
        ...
        \item Entraîné \textit{from scratch} sur une base de données de peptides connus et leurs propriétés
...
\begin{frame}
    ...
    \begin{block}{VAE et CVAE}
        \begin{itemize}
        \item Génération de peptides aléatoires à l'aide du VAE, au format code peptidique (N-terminal vers C-terminal).
        \end{itemize}
        \centering
        ex: IWIGRK
...
\begin{frame}
    \persondotright[4pt]{EduardColor}
    \frametitle{Évaluation du générateur}
    \begin{block}{Évaluation du générateur}
        ...
        \end{itemize}
        \centering
        Lorsqu'on donne la sequence KLFKRWKHLFR (peptide connu), une taille 11 et des propriétés physico-chimiques au CVAE, il génère la sequence RKLKLFFRLLR avec des propriétés similaires (selon nos agents).

        \begin{itemize}
        \item Utilisation de métriques telles que la diversité et la validité
        \end{itemize}
    \end{block}
\end{frame}
